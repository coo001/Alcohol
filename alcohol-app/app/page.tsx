...
import { DRINKS, AMBIENCES, RANDOM_EVENTS, type Drink, type DrinkId, type DrinkState, type AmbienceId } from './data';

// ─── StartScreen ────────────────────────────────────────────────────────────

function StartScreen({ onStart }: { onStart: (drinkId: DrinkId, ambienceId: AmbienceId) => void }) {
  const [selectedDrink, setSelectedDrink] = useState<DrinkId>('soju');
  const [selectedAmbience, setSelectedAmbience] = useState<AmbienceId>('pocha');
  ...
  return (
    <motion.div
      key="start"
      className={`h-screen bg-gradient-to-b ${ambience.bg} flex flex-col items-center justify-center px-6 transition-all duration-500`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <motion.h1
        className="text-white/70 text-3xl font-light tracking-[0.4em] mb-2"
      ...
      <motion.p
        className="text-white/30 text-xs tracking-[0.3em] mb-14"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ delay: 0.2 }}
      >
        {ambience.ambientText}
      </motion.p>

      <div className="w-full max-w-sm mb-7">
        <p className="text-white/30 text-xs tracking-[0.25em] mb-3">무엇을 마실까</p>
        <div className="grid grid-cols-4 gap-2">
          {drinkIds.map((id) => (
            <motion.button
              key={id}
              whileTap={{ scale: 0.94 }}
              onClick={() => setSelectedDrink(id)}
              className={`py-3 rounded-xl text-sm font-medium border transition-all duration-200 ${
                selectedDrink === id
                  ? 'bg-white/22 border-white/40 text-white shadow-sm'
                  : 'bg-white/[0.05] border-white/10 text-white/40 hover:bg-white/10 hover:text-white/60'
              }`}
            >
              {DRINKS[id].name}
            </motion.button>
      ...
      <div className="w-full max-w-sm mb-12">
        <p className="text-white/30 text-xs tracking-[0.25em] mb-3">어디서 마실까</p>
        <div className="grid grid-cols-4 gap-2">
          {ambienceIds.map((id) => (
            <motion.button
              key={id}
              whileTap={{ scale: 0.94 }}
              onClick={() => setSelectedAmbience(id)}
              className={`py-3 rounded-xl text-sm font-medium border transition-all duration-200 ${
                selectedAmbience === id
                  ? 'bg-white/22 border-white/40 text-white shadow-sm'
                  : 'bg-white/[0.05] border-white/10 text-white/40 hover:bg-white/10 hover:text-white/60'
      ...
      <motion.button
        whileTap={{ scale: 0.97 }}
        onClick={() => onStart(selectedDrink, selectedAmbience)}
        className="w-full max-w-sm py-4 rounded-2xl bg-white/14 border border-white/24 text-white text-base tracking-[0.25em] hover:bg-white/20 transition-all duration-200"
      >
        시작하기
      </motion.button>
    </motion.div>
...
function BottleVisual({ drink, pouring }: { drink: Drink; pouring: boolean }) {
  ...
  const body = drink.id === 'beer'
    ? <svg viewBox="0 0 62 162" className="w-[216px] h-[42rem]" overflow="visible"><defs><linearGradient id="terra-body" x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stopColor="rgba(255,255,255,0.2)" /><stop offset="20%" stopColor="#1a7040" /><stop offset="65%" stopColor="#0a4e2c" /><stop offset="100%" stopColor="#062e18" /></linearGradient></defs><ellipse cx="31" cy="8" rx="11" ry="4.5" fill="#e8a020" /><rect x="20" y="5" width="22" height="7" fill="#d4921a" /><ellipse cx="31" cy="5" rx="11" ry="4" fill="#f5be30" /><path d="M20 7 L22 4 L24 7 L26 4 L28 7 L30 4 L32 7 L34 4 L36 7 L38 4 L40 7 L42 4 L44 7" stroke="#d4921a" strokeWidth="1" fill="none" /><rect x="24" y="8" width="14" height="38" rx="4" fill="#0a4e2c" /><rect x="24" y="20" width="14" height="16" rx="2" fill="#f5edd8" opacity="0.95" /><text x="31" y="31" textAnchor="middle" fill="#0d7a40" fontSize="7" fontWeight="900">T</text><path d="M13 46 Q17 42 24 42 H38 Q45 42 49 46 L51 52 Q44 46 31 46 Q18 46 11 52 Z" fill="#0a4e2c" /><path d="M11 50 L51 50 L56 150 Q47 160 31 160 Q15 160 6 150 Z" fill="url(#terra-body)" /><rect x="12" y="62" width="38" height="64" rx="4" fill="#f5f0e4" opacity="0.97" /><polygon points="16,70 31,86 46,70 42,70 31,82 20,70" fill="#1a7a38" /><polygon points="16,76 31,92 46,76 42,76 31,88 20,76" fill="#1a7a38" opacity="0.45" /><text x="31" y="112" textAnchor="middle" fill="#1a1a1a" fontSize="11" fontWeight="900" letterSpacing="1.2">TERRA</text><text x="31" y="121" textAnchor="middle" fill="#555" fontSize="3.8" fontWeight="600" letterSpacing="0.5">FROM AUS</text><path d="M17 52 C15 78 16 120 20 148" stroke="rgba(255,255,255,0.14)" strokeWidth="3" strokeLinecap="round" /><ellipse cx="31" cy="156" rx="24" ry="5" fill="rgba(0,0,0,0.18)" /></svg>
    : drink.id === 'whiskey'
    ? <svg viewBox="0 0 78 150" className="w-60 h-[42rem]" overflow="visible"><defs><linearGradient id="jimbeam-amber" x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stopColor="rgba(255,255,255,0.38)" /><stop offset="22%" stopColor="rgba(210,155,75,0.78)" /><stop offset="68%" stopColor="rgba(175,115,45,0.85)" /><stop offset="100%" stopColor="rgba(130,80,25,0.62)" /></linearGradient><linearGradient id="jimbeam-fill" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#c8823a" /><stop offset="100%" stopColor="#7a4418" /></linearGradient></defs><rect x="29" y="0" width="20" height="7" rx="3.5" fill="#111" /><rect x="27" y="6" width="24" height="14" rx="3" fill="#1a1a1a" /><rect x="27" y="6" width="24" height="6" rx="2" fill="#2a2a2a" /><rect x="28" y="19" width="22" height="16" rx="3" fill="rgba(190,130,60,0.9)" /><path d="M12 35 Q16 33 28 33 H50 Q62 33 66 35 L68 44 Q60 36 39 36 Q18 36 10 44 Z" fill="rgba(175,115,45,0.9)" /><rect x="10" y="42" width="58" height="98" rx="5" fill="url(#jimbeam-fill)" /><rect x="10" y="42" width="58" height="98" rx="5" fill="url(#jimbeam-amber)" /><rect x="12" y="48" width="54" height="82" rx="3" fill="white" opacity="0.97" /><rect x="12" y="48" width="54" height="10" rx="3" fill="#cc1515" /><text x="39" y="78" textAnchor="middle" fill="#111" fontSize="11" fontWeight="900" fontStyle="italic">Jim</text><text x="39" y="92" textAnchor="middle" fill="#111" fontSize="11" fontWeight="900" fontStyle="italic">Beam</text><text x="39" y="105" textAnchor="middle" fill="#333" fontSize="4" fontWeight="700" letterSpacing="0.4">KENTUCKY STRAIGHT</text><text x="39" y="116" textAnchor="middle" fill="#cc1515" fontSize="7.5" fontWeight="900">Bourbon</text><text x="39" y="124" textAnchor="middle" fill="#333" fontSize="4.2" fontWeight="600">WHISKEY</text><path d="M18 44 C16 72 17 112 22 136" stroke="rgba(255,255,255,0.24)" strokeWidth="3.5" strokeLinecap="round" /><ellipse cx="39" cy="142" rx="30" ry="4.5" fill="rgba(0,0,0,0.2)" /></svg>
    : drink.id === 'yangju'
    ? <svg viewBox="0 0 80 162" className="w-40 h-[30rem]" overflow="visible"><defs><linearGradient id="mak-pet-body" x1="0" x2="1" y1="0" y2="0"><stop offset="0%" stopColor="rgba(255,255,255,0.7)" /><stop offset="35%" stopColor="rgba(240,237,224,0.92)" /><stop offset="75%" stopColor="rgba(215,210,192,0.82)" /><stop offset="100%" stopColor="rgba(185,180,160,0.65)" /></linearGradient><linearGradient id="mak-label-col" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#2e8c38" /><stop offset="50%" stopColor="#1e6a28" /><stop offset="100%" stopColor="#144e1e" /></linearGradient></defs><ellipse cx="40" cy="6" rx="15" ry="5.5" fill="#1a7030" /><rect x="25" y="3" width="30" height="9" rx="4.5" fill="#1a7030" /><ellipse cx="40" cy="3" rx="15" ry="5" fill="#2a9040" /><rect x="25" y="3" width="30" height="4" rx="2" fill="rgba(255,255,255,0.2)" /><rect x="28" y="11" width="24" height="16" rx="4" fill="#e4e0ce" /><path d="M12 27 Q18 24 28 24 H52 Q62 24 68 27 L70 35 Q60 28 40 28 Q20 28 10 35 Z" fill="#ddd8c4" /><rect x="10" y="32" width="60" height="116" rx="6" fill="url(#mak-pet-body)" /><rect x="10" y="46" width="60" height="84" rx="4" fill="url(#mak-label-col)" /><path d="M15 52 Q18 48 21 52 M59 52 Q62 48 65 52" stroke="#7acc80" strokeWidth="1.5" fill="none" opacity="0.7" /><circle cx="40" cy="64" r="10" fill="white" opacity="0.95" /><text x="40" y="62" textAnchor="middle" fill="#cc2020" fontSize="4" fontWeight="800">서울</text><text x="40" y="70" textAnchor="middle" fill="#1e6a28" fontSize="3.5" fontWeight="600">막걸리</text><text x="40" y="100" textAnchor="middle" fill="white" fontSize="20" fontWeight="900" letterSpacing="2">장수</text><text x="40" y="116" textAnchor="middle" fill="#c8f0c8" fontSize="8.5" fontWeight="700" letterSpacing="0.5">생막걸리</text><rect x="10" y="126" width="60" height="4" fill="#144e1e" opacity="0.6" rx="2" /><path d="M18 34 C16 62 17 106 21 140" stroke="rgba(255,255,255,0.5)" strokeWidth="5" strokeLinecap="round" /><ellipse cx="40" cy="150" rx="32" ry="5" fill="rgba(0,0,0,0.14)" /></svg>
    : null;
...
function SceneDecor({ ambienceId }: { ambienceId: AmbienceId }) {
  const scene = AMBIENCES[ambienceId];
  return (
    <div
      className="absolute inset-x-0 top-0 h-44 pointer-events-none"
      style={{ background: `radial-gradient(circle at top, ${scene.lightColor}44, transparent 62%)` }}
    />
  );
}
...
function MainScreen({
  ...
  const [currentDrink, setCurrentDrink] = useState<DrinkId>(initialDrinkId);
  ...
  const [statusText, setStatusText] = useState(AMBIENCES[ambienceId].ambientText);
  ...
  useEffect(() => {
    if (fillLevel <= 0.05 && glassState === 'empty' && !drinkChanging && !autoRefillScheduled.current) {
      ...
      const t2 = setTimeout(() => {
        setFillLevel(0.92);
        fmAnimate(visualFill, 0.92, { duration: 0.45, ease: 'easeOut' });
        setGlassState('poured');
        setIsPouring(false);
        setAutoRefillPending(false);
        autoRefillScheduled.current = false;
        flash(scene.ambientText);
      }, 1450);
      return () => { clearTimeout(t1); clearTimeout(t2); autoRefillScheduled.current = false; };
    }
  }, [drink.pourLabel, drinkChanging, fillLevel, flash, glassState, scene.ambientText, visualFill]);
  ...
  const handleShot = () => {
    ...
    setTimeout(() => {
      ...
      setFillLevel(0);
      setDrinkCounts((prev) => ({ ...prev, [drinkId]: nextCount }));
      setGlassState('empty');
      flash(drink.moodLines[Math.min(nextCount - 1, drink.moodLines.length - 1)]);
      triggerRandomEvent();
    }, 980);
  };
  ...
  const handleSip = () => {
    ...
    setTimeout(() => {
      ...
      setFillLevel(next);
      setDrinkCounts((prev) => isGlassEmpty ? { ...prev, [drinkId]: nextCount } : prev);
      setGlassState(isGlassEmpty ? 'empty' : 'poured');
      flash(drink.moodLines[Math.min(nextCount, drink.moodLines.length - 1)]);
      triggerRandomEvent();
    }, 860);
  };
  ...
  const handleToast = () => {
    ...
    setIsCheersing(true);
    setGlassState('toasting');
    flash('짠!');
    setTimeout(() => {
      setIsCheersing(false);
      setGlassState('poured');
      setFillLevel((prev) => Math.max(prev - 0.06, 0));
      flash(scene.reactionTone[Math.floor(Math.random() * scene.reactionTone.length)]);
    }, 1350);
  };
  ...
  const handleChangeDrink = (id: DrinkId) => {
    ...
    setTimeout(() => {
      ...
      setGlassState('empty');
      setFillLevel(0);
      setStatusText(scene.ambientText);
      setFlashKey((prev) => prev + 1);
      setDrinkChanging(false);
    }, 280);
  };
  ...
  return (
    <motion.div
      key="main"
      className={`h-screen bg-gradient-to-b ${scene.bg} flex flex-col overflow-hidden relative`}
      ...
      transition={{ duration: 0.4 }}
    >
      <SceneDecor ambienceId={ambienceId} />

      {/* 헤더 */}
      <div className="relative z-10 flex items-center justify-between px-5 pt-5 pb-2">
        <button
          onClick={onExit}
          className="text-white/25 text-sm tracking-[0.2em] hover:text-white/50 transition-colors"
        >
          {scene.name}
        </button>
      ...
      {/* 상태 텍스트 */}
      <div className="min-h-12 flex items-center justify-center px-8 mt-1 mb-1">
        <AnimatePresence mode="wait">
          <motion.p
            ...
            className="text-white/60 text-base tracking-wide leading-relaxed text-center"
          >
            {statusText}
          </motion.p>
        </AnimatePresence>
      ...
      {newSnack && (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-white/35 text-sm mb-1">
          {newSnack}
        </motion.p>
      )}

      {/* 음료 선택 */}
      <div className="relative z-20 w-full px-5 mt-1">
        <div className="flex gap-2 justify-center flex-wrap">
          {drinkIds.map((id) => (
            <motion.button
              key={id}
              whileTap={{ scale: 0.95 }}
              onClick={() => handleChangeDrink(id)}
              className={`px-4 py-2.5 rounded-full text-sm font-medium transition-all duration-200 border ${currentDrink === id ? 'bg-white/18 border-white/30 text-white/90' : 'bg-white/[0.05] border-white/10 text-white/40 hover:bg-white/10 hover:text-white/65'}`}
            >
              {DRINKS[id].name}
            </motion.button>
          ))}
      ...
      <div className="flex-1 min-h-0 flex flex-col items-center justify-end overflow-hidden">
        <div className="w-full relative" style={{ background: `linear-gradient(to bottom, ${scene.tableColor}99, ${scene.tableColor}f2)` }}>
          <div className="absolute right-4 top-3">
            <div className="w-28 h-14 rounded-2xl bg-white/5 border border-white/[0.08] flex items-center justify-center">
              <span className="text-white/30 text-sm text-center leading-tight px-2">{scene.snack.split(',')[0].trim()}</span>
            </div>
          </div>
          <AnimatePresence>
            {autoRefillPending && (
              <motion.div
                ...
                exit={{ opacity: 0 }}
                className="absolute top-3 left-5 text-white/35 text-sm tracking-[0.2em]"
              >
                자동 리필 중
              </motion.div>
            )}
          ...
          <div className="flex items-end justify-center pt-8 pb-6 px-5 scale-[0.78] origin-bottom">
            <div className="relative flex items-end gap-28">
              <AnimatePresence mode="wait">
                <motion.div
                  key={`bottle-${currentDrink}`}
                  initial={{ opacity: 0, scale: 0.88 }}
                  animate={{ opacity: drinkChanging ? 0 : 1, scale: drinkChanging ? 0.88 : 1 }}
                  transition={{ duration: 0.25 }}
                >
                  <BottleVisual drink={drink} pouring={isPouring} />
                </motion.div>
              </AnimatePresence>

              <AnimatePresence>
                {isPouring && <PourStream key={`stream-${currentDrink}`} drink={drink} />}
              </AnimatePresence>

              <div className="flex flex-col items-center gap-4">
                <AnimatePresence mode="wait">
                  <motion.div
                    key={`glass-${currentDrink}`}
                    initial={{ opacity: 0, scale: 0.85 }}
                    animate={{ opacity: drinkChanging ? 0 : 1, scale: drinkChanging ? 0.85 : 1 }}
                    transition={{ duration: 0.25 }}
                  >
                    <GlassVisual drink={drink} fillLevel={renderFill} state={glassState} />
                  </motion.div>
                </AnimatePresence>
                <div className="flex gap-3">
                  {[0.2, 0.45, 0.7, 0.95].map((level) => (
                    <div key={level} className={`w-3 h-3 rounded-full transition-all duration-500 ${fillLevel >= level ? 'bg-white/55' : 'bg-white/10'}`} />
                  ))}
                </div>
              </div>
            </div>
            ...
            <AnimatePresence>
              {isCheersing && <CheersGlass key="cheers-glass" drink={drink} />}
              {isCheersing && <ClinkEffect key="clink-effect" />}
            </AnimatePresence>
          </div>
        </div>

        {/* 액션 버튼 */}
        <div className="w-full max-w-2xl px-5 mt-3 pb-4 grid grid-cols-3 gap-3 flex-shrink-0">
          <ActionBtn onClick={handleShot} disabled={fillLevel === 0 || glassState === 'drinking' || isPouring || autoRefillPending} label="원샷" active={glassState === 'drinking'} />
          <ActionBtn onClick={handleSip} disabled={fillLevel === 0 || glassState === 'drinking' || isPouring || autoRefillPending} label="한모금" active={glassState === 'drinking'} highlight />
          <ActionBtn onClick={handleToast} disabled={fillLevel === 0 || isCheersing || glassState === 'drinking' || autoRefillPending} label="짠!" active={isCheersing} />
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <AnimatePresence mode="wait">
      {screen === 'start' ? (
        ...
        <MainScreen key="main" initialDrinkId={drinkId} ambienceId={ambienceId} onExit={() => setScreen('start')} />
      )}
    </AnimatePresence>
  );
}
